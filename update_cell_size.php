...
switch(optional_param('update_type', null, PARAM_TEXT)){
    case 'resize_h':
        if ($DB->record_exists('tables_sheets_rows', $data)){
            $row = $DB->get_record('tables_sheets_rows', $data, '*', MUST_EXIST);
            $row->height = optional_param('height', 0, PARAM_INT);
            $row->timemodified = time();
            $DB->update_record('tables_sheets_rows', $row);
        }
        else {
    ...
    case 'resize_w': // Updating column
        if ($DB->record_exists('tables_sheets_columns', $data)){
            $column = $DB->get_record('tables_sheets_columns', $data, '*', MUST_EXIST);
            $column->width = optional_param('width', 0, PARAM_INT);
            $column->timemodified = time();
            $DB->update_record('tables_sheets_columns', $column);
        }
        else {
